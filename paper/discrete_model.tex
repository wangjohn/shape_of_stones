...
\begin{enumerate}
  ...
  \item Select a point $\bvec{p}_1$ at random from $\bvec{l}_1$. To do this, we select a random $t \in [0,1]$ such that $\bvec{p}_1 = \bvec{v}_j t + (1-t) \bvec{v_{{j-1} \pmod{n}}$. The distribution of $t$ is defined by the probability distribution $\mathrm{P}$ so that $t \sim \mathrm{P}$.
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[keepaspectratio]{area_chipper_sample.pdf}
  \end{center}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[keepaspectratio]{distance_from_centroid.pdf}
  \end{center}
...
\begin{figure}[H]
  \begin{center}
    \includegraphics[keepaspectratio]{distance_from_centroid_skewed.pdf}
  \end{center}
